...
export async function renderOrderDetailsPage(shop: string, id: string): Promise<string> {
  ...
  const tRows = details.order.tickets.map((t) => `<tr><td><a href="/admin/tickets/${t.id}?shop=${encodeURIComponent(shop)}">${t.ticketNumber}</a></td><td>${t.status}</td><td>${esc(t.customerEmail)}</td><td>${t.createdAt.toISOString()}</td></tr>`).join('');
  const logs = details.logs.map((l) => `<li>${l.createdAt.toISOString()} - ${l.action} ${esc(l.entityType)} ${esc(l.entityId)}</li>`).join('');
